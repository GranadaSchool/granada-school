...
function SideNav({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  return(<>
    ...
    <div style={{position:"fixed",top:0,right:0,bottom:0,zIndex:200,width:"100%",maxWidth:900,display:"flex",flexDirection:"column",background:"#346988",transform:open?"translateX(0)":"translateX(100%)",transition:"transform 0.5s cubic-bezier(0.77,0,0.175,1)",boxShadow:"-8px 0 60px rgba(0,0,0,0.3)"}}>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"clamp(0.8rem,1.5vw,1.1rem) clamp(1rem,2.5vw,2.5rem)",borderBottom:"1px solid rgba(255,255,255,0.1)",flexShrink:0,gap:"1.5rem",background:"rgba(0,0,0,0.2)",flexWrap:"wrap"}}>
        ...
        <div style={{flex:1,maxWidth:280,position:"relative"}}>
          <input type="text" placeholder="Search…" value={search} onChange={e=>setSearch(e.target.value)} onFocus={()=>setSf(true)} onBlur={()=>setTimeout(()=>setSf(false),150)}
            style={{width:"100%",background:"#fff",border:`1px solid ${sf?P:"#dddbd8"}`,color:"#1c1b1c",padding:"0.45rem 2rem 0.45rem 0.8rem",fontSize:"0.78rem",outline:"none",transition:"border-color 0.2s",fontFamily:"inherit"}}/>
          <span style={{position:"absolute",right:"0.7rem",top:"50%",transform:"translateY(-50%)",color:"#aaa",fontSize:"0.85rem",pointerEvents:"none"}}>⌕</span>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden"}}>
        <div style={{width:"clamp(120px,22vw,260px)",flexShrink:0,borderRight:"1px solid rgba(255,255,255,0.1)",overflowY:"auto",padding:"1.5rem 0",background:"rgba(0,0,0,0.2)"}}>
          {NAV_ITEMS.map((item,i)=>(
            <button key={item.label} onMouseEnter={()=>setActive(i)} onClick={()=>setActive(i)}
              ...
              <span style={{fontSize:"0.8rem",fontWeight:active===i?700:400,letterSpacing:"0.06em",textTransform:"uppercase",color:active===i?S:"#fff",transition:"color 0.2s",fontFamily:"inherit"}}>{item.label}</span>
            ...
            <div style={{flex:1,overflowY:"auto",padding:"1.25rem 1.5rem"}}>
              <div style={{display:"flex",flexDirection:"column",gap:"0"}}>
                {cur.children?.map((child,i)=>(
                  <a key={i} href={child.href} onClick={onClose} style={{display:"flex",alignItems:"center",gap:"0.6rem",padding:"0.55rem 0",color:"#fff",textDecoration:"none",fontSize:"0.8rem",borderBottom:"1px solid rgba(255,255,255,0.1)",transition:"color 0.2s"}}
              ...
              <div style={{display:"flex",gap:"0.6rem",marginTop:"1.5rem",flexWrap:"wrap"}}>
                <a href="/granada-school/admissions" onClick={onClose} style={{fontSize:"0.6rem",padding:"0.45rem 1.2rem",background:S,color:P,textDecoration:"none",border:"none",cursor:"pointer",transition:"all 0.2s"}} onMouseEnter={e=>(e.currentTarget.style.opacity="0.9")} onMouseLeave={e=>(e.currentTarget.style.opacity="1")}>Enquire</a>
                <a href="/granada-school/admissions#process" onClick={onClose} style={{fontSize:"0.6rem",padding:"0.45rem 1.2rem",background:"transparent",color:"#fff",border:`1px solid ${S}`,textDecoration:"none",cursor:"pointer",transition:"all 0.2s"}} onMouseEnter={e=>{e.currentTarget.style.background=S;e.currentTarget.style.color=P;}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";e.currentTarget.style.color="#fff";}}>Apply Now</a>
              </div>
            </div>
          </div>
          {/* Right column - Image */}
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",padding:"0.85rem 2.5rem",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,gap:"1rem",flexWrap:"wrap",background:"rgba(0,0,0,0.2)"}}>
...
function PageHero(){
  ...
  return(
    <section style={{position:"relative",height:"62vh",minHeight:480,overflow:"hidden"}}>
      ...
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to right,rgba(52,105,136,0.82) 40%,rgba(52,105,136,0.3) 100%)"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(39,83,106,0.65) 0%,transparent 50%)"}}/>
      <div style={{position:"relative",zIndex:5,height:"100%",maxWidth:1280,margin:"0 auto",padding:"0 clamp(1rem,2vw,2rem)",paddingTop:"clamp(140px,16vw,165px)",display:"flex",flexDirection:"column",justifyContent:"flex-end",paddingBottom:"clamp(2rem,3vw,3.5rem)"}}>
        {/* Breadcrumb */}
        <div style={{display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"clamp(0.8rem,1.5vw,1.25rem)",opacity:loaded?1:0,transform:loaded?"translateY(0)":"translateY(10px)",transition:"all 0.6s ease 0.2s"}}>
          ...
          <span style={{color:"#fff",fontSize:"clamp(0.6rem,1vw,0.68rem)",letterSpacing:"0.12em",textTransform:"uppercase",fontWeight:600}}>About Us</span>
        </div>
        <p style={{color:"var(--secondary)",textTransform:"uppercase",letterSpacing:"0.28em",fontSize:"clamp(0.6rem,0.9vw,0.62rem)",fontWeight:700,marginBottom:"clamp(0.5rem,1vw,0.65rem)",opacity:loaded?1:0,transform:loaded?"translateY(0)":"translateY(10px)",transition:"all 0.6s ease 0.3s"}}>About Granada</p>
        <h1 className="font-display" style={{fontSize:"clamp(2.6rem,5.5vw,4.5rem)",fontWeight:600,lineHeight:1.05,color:"#fff",marginBottom:"0.8rem",opacity:loaded?1:0,transform:loaded?"translateY(0)":"translateY(20px)",transition:"all 0.8s ease 0.4s"}}>
          A Vibrant Learning<br/><span style={{fontStyle:"italic",color:"var(--accent-light)"}}>Community</span>
        </h1>
  ...
  return(
    <aside style={{width:"clamp(120px,12vw,180px)",flexShrink:0,position:"sticky",top:110,alignSelf:"flex-start",display:"none"}} className="section-sidebar">
      ...
      <div style={{marginTop:"clamp(1.5rem,2vw,2.25rem)",border:"1px solid #e8e6e3",padding:"clamp(1rem,1.5vw,1.4rem)"}}>
        ...
        {[{label:"How to Apply",href:"/granada-school/admissions#process"},{label:"Visit Granada",href:"/granada-school/admissions#team"},{label:"Enquire Now",href:"/granada-school#contact"}].map((l,i)=>(
...
function Welcome(){
  ...
  return(
    <section className="section-cream" id="welcome" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto",padding:"clamp(1.5rem,2vw,2rem) clamp(1rem,2vw,2rem)"}}>
      <p className="font-display" style={{fontSize:"clamp(1rem,1.8vw,1.5rem)",fontStyle:"italic",color:"var(--primary)",lineHeight:1.65,marginBottom:"clamp(1.5rem,2.5vw,2.25rem)",borderLeft:"3px solid var(--secondary)",paddingLeft:"clamp(0.9rem,1.5vw,1.4rem)",opacity:vis?1:0,transform:vis?"none":"translateY(16px)",transition:"all 0.8s ease"}}>
        At Granada, we believe every learner has the potential to achieve remarkable things — and we are here to nurture that potential every step of the way.
      </p>
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(2rem,4vw,3.5rem)",alignItems:"start"}} className="welcome-grid">
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateY(24px)",transition:"all 0.9s ease 0.1s"}}>
          ...
          <div style={{display:"flex",gap:"0.75rem",flexWrap:"wrap"}}>
            <a href="/granada-school/admissions" className="btn-solid">Enquire Now</a>
        ...
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateX(24px)",transition:"all 0.9s ease 0.2s"}}>
          <div style={{overflow:"hidden",height:"clamp(280px,40vw,400px)",marginBottom:"clamp(0.9rem,1.5vw,1.4rem)"}} className="img-hover">
          ...
          <div style={{borderLeft:"3px solid var(--primary)",paddingLeft:"clamp(0.8rem,1.5vw,1.2rem)"}}>
            <p style={{color:"var(--muted)",lineHeight:1.8,fontSize:"clamp(0.75rem,0.95vw,0.87rem)",fontStyle:"italic",marginBottom:"0.9rem"}}>
              "I can think of no greater privilege than guiding this remarkable school community. Granada exists to see every child discover who they are, what they are capable of, and how they can make a difference in the world."
            </p>
            <p style={{color:"var(--primary)",fontSize:"clamp(0.6rem,0.75vw,0.68rem)",letterSpacing:"0.14em",textTransform:"uppercase",fontWeight:700}}>
              The Principal<br/><span style={{color:"var(--muted)",fontWeight:400,textTransform:"none",letterSpacing:"normal",fontSize:"0.8rem"}}>Granada School</span>
            </p>
          </div>
        </div>
      </div>
      <style>{`@media(min-width:768px){.section-sidebar{display:block!important}.welcome-grid{grid-template-columns:1fr 1fr!important}}@media(max-width:640px){.welcome-grid{gap:clamp(1.5rem,2.5vw,2rem)!important}}`}</style>
      </div>
    </section>
  );
}
...
function VisionMission(){
  ...
  return(
    <section className="section-blue" id="vision" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(1.2rem,2vw,1.75rem)",marginBottom:"clamp(1.5rem,2.5vw,2.75rem)"}} className="vision-grid">
        {[
          ...
          <div key={i} style={{padding:"clamp(1.5rem,2.5vw,2.25rem)",border:`1px solid ${c.borderColor}`,position:"relative",overflow:"hidden",background:"rgba(255,255,255,0.06)",opacity:vis?1:0,transform:vis?"none":"translateY(32px)",transition:`all 0.9s ease ${i*0.15}s`}}>
            ...
            <span style={{display:"inline-block",fontSize:"0.58rem",letterSpacing:"0.2em",textTransform:"uppercase",color:c.tagColor,fontWeight:700,marginBottom:"0.9rem",border:`1px solid ${c.tagColor}`,padding:"0.25rem 0.7rem"}}>{c.tag}</span>
            <h3 className="font-display" style={{fontSize:"1.55rem",fontWeight:500,color:"#fff",lineHeight:1.2,marginBottom:"0.9rem"}}>{c.title}</h3>
      ...
      <div style={{opacity:vis?1:0,transform:vis?"none":"translateY(24px)",transition:"all 0.9s ease 0.3s"}}>
        <p className="body-text" style={{marginBottom:"1.75rem",maxWidth:620}}>Our strategic pillars bring this vision to life — translating the Granada ethos into everyday learning, personal growth, and inspiring experiences.</p>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"1px",background:"rgba(255,255,255,0.12)"}} className="vision-pillars">
          {[
            {title:"Character & Wellbeing",desc:"Cultivating resilient, ethical, and socially aware learners across all aspects of school life.",icon:"◆"},
            {title:"Academic Excellence",  desc:"Delivering personalised, high-performance learning that equips pupils for university and beyond.", icon:"◇"},
            {title:"Community & Citizenship",desc:"Fostering a culture of service, leadership, and responsibility both locally and globally.",       icon:"◈"},
          ].map((p,i)=>(
            <div key={i} style={{padding:"1.75rem 1.5rem",background:"rgba(255,255,255,0.04)"}}>
              <span style={{fontSize:"0.95rem",color:"var(--secondary)",display:"block",marginBottom:"0.65rem"}}>{p.icon}</span>
              <h4 style={{fontSize:"0.75rem",textTransform:"uppercase",letterSpacing:"0.12em",color:"#fff",fontWeight:700,marginBottom:"0.6rem"}}>{p.title}</h4>
              <p style={{color:"rgba(255,255,255,0.72)",fontSize:"0.8rem",lineHeight:1.75,fontWeight:300}}>{p.desc}</p>
            </div>
          ))}
        </div>
      </div>
...
function CoreValues(){
  ...
  const values=[
    ...
    {title:"Curiosity",icon:"🔭",color:"var(--secondary)",tagline:"Love of Learning",desc:"We foster a deep love for learning, creativity, and the fearless exploration of new ideas. At Granada, questions are always welcome — curiosity drives discovery, innovation, and lifelong learning.",extras:["Inquiry-based learning","Creative problem-solving","Cross-curricular exploration"]},
    ...
    {title:"Respect",icon:"🌍",color:"var(--accent-blue)",tagline:"Valuing Every Voice",desc:"We celebrate diversity, nurture positive relationships, and create a supportive environment where every individual is seen and valued. Respect connects our community across backgrounds and aspirations.",extras:["Cultural celebrations","Student voice programmes","Anti-bullying commitment"]},
  ];
  return(
    <section className="section-cream" id="values" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      ...
      <div style={{marginBottom:"clamp(1.5rem,2.5vw,2.75rem)",opacity:vis?1:0,transform:vis?"none":"translateY(20px)",transition:"all 0.8s ease"}}>
        ...
        <div className="divider"/>
        <p className="body-text" style={{maxWidth:560}}>Five key values shape every interaction, every lesson, and every ambition we hold for our learners.</p>
      </div>
...
function OurStory(){
  const {ref,vis}=useInView(0.08);
  return(
    <section className="section-cream" id="history" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto",padding:"clamp(1.5rem,2vw,2rem) clamp(1rem,2vw,2rem)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(2rem,4vw,4rem)",alignItems:"center"}} className="story-grid">
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"clamp(180px,30vw,220px) clamp(140px,22vw,175px)",gap:8,opacity:vis?1:0,transform:vis?"none":"translateX(-24px)",transition:"all 0.9s ease"}}>
          {[
            ...
            {src:"/staffroom.jpeg",col:"2/3",row:"1/3"},
      ...
      <style>{`@media(max-width:768px){#history>div>div{grid-template-columns:1fr!important}}`}</style>
      </div>
    </section>
  );
}
...
function Boarding(){
  ...
  return(
    <section className="section-blue" id="boarding" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto",padding:"clamp(1.5rem,2vw,2rem) clamp(1rem,2vw,2rem)"}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(2rem,4vw,4rem)",alignItems:"center",marginBottom:"clamp(2rem,3vw,3rem)"}} className="boarding-grid">
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateX(-24px)",transition:"all 0.9s ease"}}>
          ...
          <div className="divider"/>
          <p className="body-text" style={{marginBottom:"1.1rem"}}>Granada provides boarding facilities for Junior and Senior School girls — a safe, structured, and nurturing environment designed for focused learning and personal growth.</p>
          ...
          <p className="body-text" style={{marginBottom:"2rem"}}>From weekend activities to cultural events, boarding life at Granada shapes independence, resilience, and lifelong friendships — preparing girls for the challenges and opportunities that lie ahead.</p>
          <a href="/granada-school/admissions#boarding" className="btn-solid">Enquire About Boarding</a>
        </div>
        <div style={{overflow:"hidden",height:"clamp(300px,50vw,420px)",opacity:vis?1:0,transform:vis?"none":"translateX(24px)",transition:"all 0.9s ease 0.15s"}} className="img-hover">
      ...
      <style>{`@media(max-width:768px){#boarding>div>div:first-of-type{grid-template-columns:1fr!important}#boarding>div>div:last-child{grid-template-columns:1fr 1fr!important}}@media(max-width:500px){#boarding>div>div:last-child{grid-template-columns:1fr!important}}`}</style>
      </div>
    </section>
  );
}
  ...
  return(
    <section className="section-cream" id="staff" ref={ref} style={{paddingTop:"clamp(3rem,6vw,5rem)",paddingBottom:"clamp(3rem,6vw,5rem)",scrollMarginTop:"100px"}}>
      ...
      <div style={{marginBottom:"clamp(1.5rem,2.5vw,2.5rem)",opacity:vis?1:0,transform:vis?"none":"translateY(16px)",transition:"all 0.8s ease"}}>
        ...
        <div className="divider"/>
        <p className="body-text" style={{maxWidth:560}}>Our dedicated team of educators brings together expertise, passion, and a shared commitment to every learner's journey.</p>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:"clamp(0.9rem,1.5vw,1.4rem)"}}>
        {staff.map((s,i)=>(
          <div key={i} style={{opacity:vis?1:0,transform:vis?"none":"translateY(32px)",transition:`all 0.9s ease ${i*0.1}s`}}>
            <div className="img-hover" style={{overflow:"hidden",height:"clamp(220px,35vw,280px)",marginBottom:"clamp(0.6rem,1vw,0.9rem)"}}>
...
function Footer(){
  return(
    <footer id="contact" className="section-blue" style={{padding:"clamp(3rem,4vw,4.5rem) clamp(1rem,2vw,2rem) clamp(1.2rem,2vw,2rem)"}}>
      ...
      <style>{`@media(min-width:640px){.footer-grid{grid-template-columns:1fr 1fr 1fr 1fr!important}.story-grid{grid-template-columns:1fr 1fr!important}.boarding-grid{grid-template-columns:1fr 1fr!important}}@media(max-width:500px){#values>div>div:nth-of-type(2){grid-template-columns:1fr!important}}`}</style>
    </footer>
  );
}
