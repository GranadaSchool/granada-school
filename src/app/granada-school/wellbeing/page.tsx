  ...
  return(<>
    ...
    <div style={{position:"fixed",top:0,right:0,bottom:0,zIndex:200,width:"100%",maxWidth:"min(100vw,900px)",display:"flex",flexDirection:"column",background:"#346988",transform:open?"translateX(0)":"translateX(100%)",transition:"transform 0.5s cubic-bezier(0.77,0,0.175,1)",boxShadow:"-8px 0 60px rgba(0,0,0,0.3)"}}>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",padding:"0.85rem 2.5rem",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0,flexWrap:"wrap",gap:"0.75rem",background:"rgba(0,0,0,0.2)"}}>
        ...
        <p style={{color:"rgba(255,255,255,0.8)",fontSize:"0.7rem",letterSpacing:"0.06em"}}>Vipingo, Kilifi County, Kenya</p>
      </div>
    </div>
  </>);
}
...
function PageHero(){
  ...
  return(
    <section style={{position:"relative",height:"65vw",minHeight:480,overflow:"hidden"}}>
      <div style={{position:"absolute",inset:0,backgroundImage:"url(/sports2.jpeg)",backgroundSize:"cover",backgroundPosition:"center",animation:"kbWellbeing 14s ease-in-out infinite alternate"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(74,100,40,0.88) 0%,rgba(52,105,136,0.65) 55%,rgba(13,12,13,0.45) 100%)"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(13,12,13,0.55) 0%,transparent 50%)"}}/>
      <div style={{position:"absolute",top:"15%",right:"8%",width:"clamp(100px,15vw,190px)",height:"clamp(100px,15vw,190px)",borderRadius:"50%",border:"1px solid rgba(170,194,12,0.25)",animation:"spinW 30s linear infinite",opacity:0.6}}/>
      <div style={{position:"absolute",bottom:"20%",left:"3%",width:"clamp(70px,10vw,130px)",height:"clamp(70px,10vw,130px)",borderRadius:"50%",border:"1px solid rgba(255,255,255,0.1)",animation:"spinW 20s linear infinite reverse",opacity:0.4}}/>
      <div style={{position:"relative",zIndex:5,height:"100%",maxWidth:1280,margin:"0 auto",padding:"0 clamp(1rem,2vw,2rem)",paddingTop:"clamp(140px,16vw,165px)",display:"flex",flexDirection:"column",justifyContent:"flex-end",paddingBottom:"clamp(2.5rem,4.5vw,4.5rem)"}}>
        <div style={{display:"flex",alignItems:"center",gap:"0.45rem",marginBottom:"1.1rem",opacity:loaded?1:0,transition:"all 0.7s ease 0.2s",flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:"0.65rem",letterSpacing:"0.12em",textTransform:"uppercase",fontWeight:600}}>Pastoral & Wellbeing</span>
        </div>
        <p style={{color:"var(--secondary)",textTransform:"uppercase",letterSpacing:"0.28em",fontSize:"0.6rem",fontWeight:700,marginBottom:"0.55rem",opacity:loaded?1:0,transition:"all 0.7s ease 0.3s"}}>Pastoral Care & Wellbeing</p>
        <h1 className="font-display" style={{fontSize:"clamp(2.4rem,5.5vw,4.8rem)",fontWeight:600,lineHeight:1.02,color:"#fff",marginBottom:"0.5rem",opacity:loaded?1:0,transform:loaded?"none":"translateY(20px)",transition:"all 0.8s ease 0.4s"}}>
          Nurturing the <br/><span style={{fontStyle:"italic",color:"var(--accent-light)"}}>Whole Girl</span>
        </h1>
        ...
        <p style={{color:"rgba(255,255,255,0.82)",fontSize:"clamp(0.84rem,1.2vw,1rem)",fontWeight:300,maxWidth:480,lineHeight:1.82,marginBottom:"1.8rem",opacity:loaded?1:0,transform:loaded?"none":"translateY(18px)",transition:"all 0.8s ease 0.55s"}}>
          Emotional wellbeing, strong character, and spiritual growth — at the heart of everything we do at Granada.
        </p>
        <div style={{display:"flex",gap:"0.9rem",flexWrap:"wrap",opacity:loaded?1:0,transition:"opacity 0.9s ease 0.7s"}}>
          <a href="#approach" className="btn-green" style={{fontSize:"0.68rem",padding:"0.75rem 2rem"}}>Our Approach</a>
          <a href="/granada-school/admissions" style={{display:"inline-block",padding:"0.75rem 2rem",background:"transparent",border:"1px solid rgba(255,255,255,0.6)",color:"#fff",textTransform:"uppercase",letterSpacing:"0.14em",fontSize:"0.68rem",fontWeight:600,textDecoration:"none",transition:"all 0.3s"}}
...
function WellbeingApproach(){
  ...
  const pillars=[
    {title:"Emotional Intelligence",desc:"We cultivate self-awareness, empathy, and emotional regulation — equipping learners to understand and manage their emotions with maturity and grace.",icon:"💛",color:"#ebae1b"},
    {title:"Mental Health Support",desc:"Dedicated counsellors and a proactive approach to mental wellbeing ensure every student has someone to talk to, support systems in place, and coping strategies to navigate life's challenges.",icon:"🌿",color:"#4a6428"},
    {title:"Physical Wellness",desc:"A structured schedule of physical activity, nutritious meals, and rest ensures each student maintains energy, health, and the strength to perform at their best.",icon:"🏃",color:"#346988"},
    {title:"Spiritual Growth",desc:"Respectful of diverse faiths, we create space for reflection, gratitude, and the development of a strong inner life and moral compass.",icon:"✨",color:"#483454"},
    {title:"Social Skills",desc:"Cooperative learning, boarding life, and shared experiences build authentic friendships, communication skills, and lifelong bonds of sisterhood.",icon:"🤝",color:"#84b7c8"},
    {title:"Academic Resilience",desc:"We teach learners how to handle setbacks, manage pressure, and develop a growth mindset — turning challenges into stepping stones for success.",icon:"📈",color:"#c0180b"},
  ];
  return(
    <section id="approach" ref={ref} className="section-cream" style={{padding:"clamp(3rem,6vw,6rem) clamp(1rem,2vw,2rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:"clamp(2rem,4vw,3.5rem)",opacity:vis?1:0,transform:vis?"none":"translateY(20px)",transition:"all 0.8s ease"}}>
          <p className="label-tag">Our Philosophy</p>
          <h2 className="section-heading">Wellbeing <em>At Granada</em></h2>
          <div className="divider" style={{margin:"1.25rem auto"}}/>
          <p className="body-text" style={{maxWidth:540,margin:"0 auto"}}>Wellbeing isn't a department at Granada — it's woven into everything we do, every day, for every girl.</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(0.9rem,1.5vw,1.3rem)"}} className="pillars-grid">
          {pillars.map((p,i)=>(
            <div key={i} style={{background:"#fff",border:"1px solid rgba(0,0,0,0.06)",padding:"clamp(1.2rem,2vw,1.8rem)",position:"relative",overflow:"hidden",opacity:vis?1:0,transform:vis?"none":`translateY(${22+i*5}px)`,transition:`all 0.8s ease ${i*0.09}s`}}
              onMouseEnter={e=>{const el=e.currentTarget;el.style.boxShadow=`0 10px 40px ${p.color}18`;el.style.borderColor=`${p.color}38`;el.style.transform="translateY(-4px)";}}
              ...
              <div style={{position:"absolute",bottom:-30,right:-20,width:100,height:100,borderRadius:"50%",background:p.color,opacity:0.04}}/>
              <div style={{display:"flex",alignItems:"flex-start",gap:"1rem"}}>
                <div style={{width:"clamp(40px,5vw,52px)",height:"clamp(40px,5vw,52px)",background:`${p.color}14`,border:`1px solid ${p.color}28`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:"clamp(1.1rem,1.6vw,1.4rem)",flexShrink:0}}>
                  {p.icon}
                </div>
                <div>
                  <h3 style={{fontSize:"clamp(0.85rem,1.15vw,0.98rem)",fontWeight:700,color:"var(--body-text)",marginBottom:"0.4rem"}}>{p.title}</h3>
                  <p className="body-text" style={{fontSize:"clamp(0.76rem,0.92vw,0.84rem)",marginBottom:0}}>{p.desc}</p>
                </div>
              </div>
            </div>
          ))}
...
function CounsellingSection(){
  const {ref,vis}=useInView(0.07);
  return(
    <section id="counselling" ref={ref} className="section-blue" style={{padding:"clamp(3rem,6vw,6rem) clamp(1rem,2vw,2rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr",gap:"clamp(2.5rem,5vw,5rem)",alignItems:"center"}} className="counsel-grid">
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateX(-28px)",transition:"all 0.9s ease"}}>
          ...
          <p className="body-text" style={{marginBottom:"1.2rem"}}>
            Granada has a dedicated professional counselling programme designed to support every learner's emotional and psychological wellbeing. Our counsellors are trained to provide compassionate, confidential support.
          </p>
          <p className="body-text" style={{marginBottom:"1.2rem"}}>
            Whether a student is navigating academic pressure, social challenges, homesickness, or personal growth questions, our dedicated counselling team is here — creating a safe, non-judgmental space for every conversation.
          </p>
          <p className="body-text" style={{marginBottom:"2rem"}}>
            We also offer group sessions, life-skills workshops, and peer support programmes that build emotional vocabulary, resilience strategies, and healthy coping skills for every stage of adolescent development.
          </p>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
            {[
              {label:"Individual Counselling",desc:"One-on-one sessions with a trained counsellor"},
              {label:"Group Sessions",desc:"Peer discussion and shared learning"},
              {label:"Life Skills Workshops",desc:"Practical emotional intelligence training"},
              {label:"Peer Support",desc:"Older students mentoring younger girls"},
            ].map((s,i)=>(
              <div key={i} style={{padding:"1rem",border:"1px solid rgba(255,255,255,0.15)",background:"rgba(255,255,255,0.06)",opacity:vis?1:0,transform:vis?"none":"translateY(12px)",transition:`all 0.7s ease ${0.3+i*0.1}s`}}>
                <p style={{color:"var(--secondary)",fontSize:"0.6rem",fontWeight:700,letterSpacing:"0.14em",textTransform:"uppercase",marginBottom:"0.3rem"}}>{s.label}</p>
                <p style={{color:"rgba(255,255,255,0.72)",fontSize:"0.78rem",lineHeight:1.6}}>{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateY(28px)",transition:"all 0.9s ease 0.2s",display:"grid",gridTemplateColumns:"1fr 1fr",gridTemplateRows:"clamp(200px,26vw,280px) clamp(130px,17vw,170px)",gap:"10px"}}>
          {[
            {src:"/staffroom.jpeg",col:"1/2",row:"1/2"},
            {src:"/sports2.jpeg",col:"2/3",row:"1/3"},
            {src:"/building.jpeg",col:"1/2",row:"2/3"},
          ].map((img,i)=>(
            <div key={i} className="img-hover" style={{gridColumn:img.col,gridRow:img.row,overflow:"hidden",position:"relative"}}>
              <img src={img.src} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
              <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(52,105,136,0.4),transparent 65%)"}}/>
            </div>
          ))}
        </div>
      </div>
      <style>{`@media(min-width:900px){.counsel-grid{grid-template-columns:1fr 1fr!important}}`}</style>
    </section>
  );
}
...
function CharacterSection(){
  const {ref,vis}=useInView(0.07);
  const values=[
    {label:"Resilience",color:"#c0180b",desc:"Facing setbacks with courage and bouncing back stronger than before."},
    {label:"Curiosity",color:"#ebae1b",desc:"Asking questions, seeking knowledge, and embracing a love of learning."},
    {label:"Kindness",color:"#4a6428",desc:"Treating every person with warmth, empathy, and genuine care."},
    {label:"Honesty",color:"#84b7c8",desc:"Speaking truth, acting with integrity in all circumstances."},
    {label:"Respect",color:"#483454",desc:"Honouring self, others, and the environment around us."},
  ];
  return(
    <section id="character" ref={ref} className="section-cream" style={{padding:"clamp(3rem,6vw,6rem) clamp(1rem,2vw,2rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:"clamp(2rem,4vw,3.5rem)",opacity:vis?1:0,transform:vis?"none":"translateY(20px)",transition:"all 0.8s ease"}}>
          <p className="label-tag">Character Formation</p>
          <h2 className="section-heading">Built On <em>Five Core Values</em></h2>
          <div className="divider" style={{margin:"1.25rem auto"}}/>
          <p className="body-text" style={{maxWidth:520,margin:"0 auto"}}>Our values aren't a list on a wall — they are lived daily, celebrated continuously, and woven into the fabric of every experience at Granada.</p>
        </div>

        <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(0.8rem,1.2vw,1.2rem)"}} className="values-grid">
          {values.map((v,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:`clamp(60px,8vw,80px) 1fr`,gap:"0",alignItems:"stretch",overflow:"hidden",opacity:vis?1:0,transform:vis?"none":`translateX(${i%2===0?-24:24}px)`,transition:`all 0.8s ease ${i*0.1}s`}}>
              <div style={{background:v.color,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"clamp(1rem,1.5vw,1.4rem) clamp(0.5rem,0.8vw,0.75rem)",textAlign:"center"}}
                onMouseEnter={e=>{e.currentTarget.style.filter="brightness(1.1)";}} onMouseLeave={e=>{e.currentTarget.style.filter="brightness(1)";}}>
                <p style={{color:"#fff",fontSize:"clamp(0.52rem,0.7vw,0.6rem)",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",writingMode:"vertical-rl",textOrientation:"mixed",transform:"rotate(180deg)"}}>{v.label}</p>
              </div>
              <div style={{background:"#fff",padding:"clamp(1rem,1.8vw,1.5rem)",border:`1px solid ${v.color}18`,borderLeft:"none",transition:"all 0.3s"}}
                onMouseEnter={e=>{e.currentTarget.style.background=`${v.color}05`;}} onMouseLeave={e=>{e.currentTarget.style.background="#fff";}}>
                <h3 style={{fontSize:"clamp(0.9rem,1.3vw,1.05rem)",fontWeight:700,color:v.color,marginBottom:"0.4rem"}}>{v.label}</h3>
                <p className="body-text" style={{fontSize:"clamp(0.78rem,0.95vw,0.86rem)",marginBottom:0}}>{v.desc}</p>
              </div>
            </div>
          ))}
        </div>
...
function GlobalSection(){
  ...
  return(
    <section id="global" ref={ref} style={{position:"relative",padding:"clamp(3.5rem,7vw,7rem) clamp(1rem,2vw,2rem)",overflow:"hidden",scrollMarginTop:"100px"}}>
      <div style={{position:"absolute",inset:0,backgroundImage:"url(/sports.jpeg)",backgroundSize:"cover",backgroundPosition:"center",filter:"brightness(0.15)"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(74,100,40,0.92) 0%,rgba(13,12,13,0.88) 100%)"}}/>
      <div style={{position:"absolute",top:"10%",right:"5%",width:"clamp(150px,20vw,280px)",height:"clamp(150px,20vw,280px)",borderRadius:"50%",border:"1px solid rgba(170,194,12,0.15)",animation:"spinW 35s linear infinite"}}/>
      <div style={{position:"relative",zIndex:2,maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr",gap:"clamp(2rem,4vw,4rem)",alignItems:"center"}} className="global-grid">
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateX(-28px)",transition:"all 0.9s ease"}}>
          <p style={{color:"var(--secondary)",textTransform:"uppercase",letterSpacing:"0.28em",fontSize:"0.62rem",fontWeight:700,marginBottom:"1.1rem"}}>Global Citizenship</p>
          <h2 className="font-display" style={{fontSize:"clamp(2rem,3.8vw,3.2rem)",fontWeight:500,color:"#fff",marginBottom:"1.1rem",lineHeight:1.1}}>Raising <em style={{color:"var(--secondary)"}}>Responsible</em><br/>Global Citizens</h2>
          <div style={{width:50,height:2,background:"var(--secondary)",marginBottom:"1.5rem"}}/>
          <p style={{color:"rgba(255,255,255,0.78)",fontSize:"clamp(0.82rem,1.1vw,0.9rem)",lineHeight:1.88,marginBottom:"1rem"}}>
            At Granada, we believe education must transcend the classroom and prepare learners to engage meaningfully with the world. Our global citizenship education instils awareness, responsibility, and compassion.
          </p>
          <p style={{color:"rgba(255,255,255,0.78)",fontSize:"clamp(0.82rem,1.1vw,0.9rem)",lineHeight:1.88,marginBottom:"2rem"}}>
            Through community service projects, environmental stewardship, cultural appreciation programmes, and discussions of global challenges, we shape learners who understand their role as responsible citizens — locally and globally.
          </p>
          <div style={{display:"flex",flexWrap:"wrap",gap:"0.6rem"}}>
            {["Community Service","Environmental Stewardship","Cultural Appreciation","Debate & Public Speaking","Social Justice Awareness","SDG Projects"].map((tag,i)=>(
              <span key={i} style={{padding:"0.35rem 0.9rem",border:"1px solid rgba(170,194,12,0.4)",color:"var(--secondary)",fontSize:"0.68rem",letterSpacing:"0.08em",transition:"all 0.25s",cursor:"default"}}
                onMouseEnter={e=>{e.currentTarget.style.background="rgba(170,194,12,0.15)";}} onMouseLeave={e=>{e.currentTarget.style.background="transparent";}}>
                {tag}
              </span>
            ))}
          </div>
        ...
        <div style={{opacity:vis?1:0,transform:vis?"none":"translateY(28px)",transition:"all 0.9s ease 0.2s"}}>
          <div className="img-hover" style={{overflow:"hidden",height:"clamp(300px,38vw,460px)",position:"relative"}}>
            <img src="/class.jpeg" alt="Global Citizenship" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(74,100,40,0.5),transparent 60%)"}}/>
          </div>
        </div>
      </div>
      <style>{`@media(min-width:900px){.global-grid{grid-template-columns:1fr 1fr!important}}`}</style>
    </section>
  );
}
  ...
  return(
    <aside style={{width:"clamp(120px,12vw,180px)",flexShrink:0,position:"sticky",top:110,alignSelf:"flex-start",display:"none"}} className="section-sidebar">
      ...
      <div style={{marginTop:"clamp(1.5rem,2vw,2.25rem)",border:"1px solid #e8e6e3",padding:"clamp(1rem,1.5vw,1.4rem)"}}>
        ...
        {[{label:"Campus Life",href:"/granada-school/campus-life"},{label:"Admissions",href:"/granada-school/admissions"},{label:"Academic Life",href:"/granada-school/academics"}].map((l,i)=>(
