...
const NAV_ITEMS = [
  {label:"About Us",href:"/granada-school/about",image:"/building.jpeg",imageCaption:"A Unique Blend of Discovery + Purpose",
    children:[{label:"Principal's Welcome",href:"/granada-school/about#welcome"},{label:"Vision & Aims",href:"/granada-school/about#vision"},{label:"Our Values",href:"/granada-school/about#values"},{label:"Our Story",href:"/granada-school/about#history"},{label:"Staff & Leadership",href:"/granada-school/about#staff"},{label:"Contact Us",href:"/granada-school#contact"}]},
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"clamp(0.8rem,2vw,1.1rem) clamp(1.5rem,3vw,2.5rem)",borderBottom:"1px solid rgba(255,255,255,0.1)",flexShrink:0,gap:"1.5rem",background:"rgba(0,0,0,0.2)",flexWrap:"wrap"}}>
        <div style={{display:"flex",gap:"1.5rem",alignItems:"center",flex:1,minWidth:0,flexWrap:"wrap"}}>
          {[{label:"Parents",href:"/granada-school#contact"},{label:"Enquire",href:"/granada-school/admissions"},{label:"Contact us",href:"/granada-school#contact"}].map(l=>(
            <a key={l.label} href={l.href} onClick={onClose} style={{color:"#fff",textDecoration:"none",fontSize:"0.7rem",letterSpacing:"0.08em",textTransform:"uppercase",fontWeight:500,transition:"color 0.2s",whiteSpace:"nowrap"}}
        ...
        <div style={{width:"clamp(180px,25vw,260px)",flexShrink:0,borderRight:"1px solid rgba(255,255,255,0.1)",overflowY:"auto",padding:"1.5rem 0",background:"rgba(0,0,0,0.2)"}}>
          {NAV_ITEMS.map((item,i)=>(
            <button key={item.label} onMouseEnter={()=>setActive(i)} onClick={()=>setActive(i)}
              ...
              <span style={{fontSize:"0.8rem",fontWeight:active===i?700:400,letterSpacing:"0.06em",textTransform:"uppercase",color:active===i?S:"#fff",transition:"color 0.2s",fontFamily:"inherit",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.label}</span>
...
function PageHero(){
  ...
  return(
    <section style={{position:"relative",height:"68vh",minHeight:500,overflow:"hidden"}}>
      {slides.map((s,i)=>(
        <div key={i} style={{position:"absolute",inset:0,backgroundImage:`url(${s})`,backgroundSize:"cover",backgroundPosition:"center",opacity:i===active?1:0,transition:"opacity 1.4s ease",animation:i===active?"kbCampus 14s ease-in-out infinite alternate":"none"}}/>
      ))}
      <div style={{position:"absolute",inset:0,background:"linear-gradient(140deg,rgba(52,105,136,0.88) 0%,rgba(52,105,136,0.45) 55%,rgba(13,12,13,0.4) 100%)"}}/>
      <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(13,12,13,0.55) 0%,transparent 50%)"}}/>
      <div style={{position:"relative",zIndex:5,height:"100%",maxWidth:1280,margin:"0 auto",padding:"0 clamp(1rem,2vw,2rem)",paddingTop:"clamp(140px,16vw,165px)",display:"flex",flexDirection:"column",justifyContent:"flex-end",paddingBottom:"clamp(2.5rem,4vw,4.5rem)"}}>
        <div style={{display:"flex",alignItems:"center",gap:"0.45rem",marginBottom:"1.1rem",opacity:loaded?1:0,transition:"all 0.7s ease 0.2s",flexWrap:"wrap"}}>
          ...
          <span style={{color:"#fff",fontSize:"0.65rem",letterSpacing:"0.12em",textTransform:"uppercase",fontWeight:600}}>Campus Life</span>
        </div>
        <p style={{color:"var(--secondary)",textTransform:"uppercase",letterSpacing:"0.28em",fontSize:"0.6rem",fontWeight:700,marginBottom:"0.55rem",opacity:loaded?1:0,transition:"all 0.7s ease 0.3s"}}>Life at Granada</p>
        <h1 className="font-display" style={{fontSize:"clamp(2.5rem,6vw,5rem)",fontWeight:600,lineHeight:1.02,color:"#fff",marginBottom:"0.5rem",opacity:loaded?1:0,transform:loaded?"none":"translateY(20px)",transition:"all 0.8s ease 0.4s"}}>
          Where Learning <br/><span style={{fontStyle:"italic",color:"var(--accent-light)"}}>Comes Alive</span>
        </h1>
        ...
        <p style={{color:"rgba(255,255,255,0.82)",fontSize:"clamp(0.84rem,1.25vw,1rem)",fontWeight:300,maxWidth:480,lineHeight:1.82,marginBottom:"1.8rem",opacity:loaded?1:0,transform:loaded?"none":"translateY(18px)",transition:"all 0.8s ease 0.55s"}}>
        ...
        <div style={{display:"flex",gap:"0.9rem",flexWrap:"wrap",opacity:loaded?1:0,transition:"opacity 0.9s ease 0.7s"}}>
          <a href="#facilities" className="btn-green" style={{fontSize:"0.68rem",padding:"0.75rem 2rem"}}>Explore Campus</a>
      ...
      <div style={{position:"absolute",bottom:"1.4rem",left:"50%",transform:"translateX(-50%)",display:"flex",gap:6,zIndex:5}}>
        {slides.map((_,i)=>(
          <button key={i} onClick={()=>setActive(i)} style={{width:i===active?"24px":"6px",height:"2px",background:i===active?"var(--secondary)":"rgba(255,255,255,0.5)",border:"none",cursor:"pointer",transition:"all 0.4s",padding:0}}/>
        ))}
      </div>
    ...
    <section id="facilities" ref={ref} className="section-cream" style={{padding:"clamp(3rem,6vw,6rem) clamp(1rem,2vw,2rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:"clamp(2rem,4vw,3.5rem)",opacity:vis?1:0,transform:vis?"none":"translateY(20px)",transition:"all 0.8s ease"}}>
          ...
          <p className="body-text" style={{maxWidth:500,margin:"0 auto"}}>Purpose-built spaces designed to inspire, challenge, and support every aspect of a student's growth.</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr",gap:"clamp(1rem,1.8vw,1.5rem)"}} className="facilities-grid">
          {facilities.map((f,i)=>(
            <div key={i} style={{background:"#fff",border:"1px solid rgba(0,0,0,0.06)",overflow:"hidden",opacity:vis?1:0,transform:vis?"none":`translateY(${24+i*5}px)`,transition:`all 0.8s ease ${i*0.09}s`,cursor:"default"}}
              onMouseEnter={e=>{e.currentTarget.style.boxShadow="0 12px 50px rgba(52,105,136,0.12)";e.currentTarget.style.transform="translateY(-4px)";}}
              onMouseLeave={e=>{e.currentTarget.style.boxShadow="none";e.currentTarget.style.transform="none";}}>
              <div className="img-hover" style={{height:"clamp(150px,16vw,190px)",overflow:"hidden",position:"relative"}}>
                <img src={f.img} alt={f.title} style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(52,105,136,0.55),transparent 60%)"}}/>
                <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:"var(--primary)"}}/>
              </div>
              <div style={{padding:"clamp(1rem,1.6vw,1.4rem)"}}>
    ...
    <section id="cocurricular" ref={ref} className="section-cream" style={{padding:"clamp(3rem,6vw,6rem) clamp(1rem,2vw,2rem)",scrollMarginTop:"100px"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:"clamp(2rem,4vw,3.5rem)",opacity:vis?1:0,transform:vis?"none":"translateY(20px)",transition:"all 0.8s ease"}}>
          ...
          <div className="divider" style={{margin:"1.25rem auto"}}/>
          <p className="body-text" style={{maxWidth:520,margin:"0 auto"}}>A rich tapestry of activities that nurture talent, build character, and make every day at Granada extraordinary.</p>
        </div>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:"0.5rem",justifyContent:"center",marginBottom:"clamp(1.5rem,2.5vw,2.5rem)",opacity:vis?1:0,transition:"opacity 0.8s ease 0.2s"}}>
          {categories.map((c,i)=>(
            <button key={i} onClick={()=>setActive(i)} style={{padding:"0.55rem 1.4rem",background:active===i?c.color:"transparent",color:active===i?"#fff":c.color,border:`1px solid ${c.color}`,cursor:"pointer",fontSize:"0.68rem",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",transition:"all 0.3s",fontFamily:"inherit"}}>
              {c.label}
            </button>
          ))}
        </div>
        ...
        {categories.map((c,i)=>(
          <div key={i} style={{display:active===i?"grid":"none",gridTemplateColumns:"1fr",gap:"clamp(1.5rem,3vw,2.5rem)",alignItems:"center",animation:"fadeInActivity 0.5s ease"}} className="activity-panel">
            <div className="img-hover" style={{overflow:"hidden",height:"clamp(240px,32vw,400px)",position:"relative"}}>
            ...
            <div>
              <p style={{color:c.color,fontSize:"0.6rem",letterSpacing:"0.22em",textTransform:"uppercase",fontWeight:700,marginBottom:"0.6rem"}}>Activities</p>
              ...
              <div style={{display:"flex",flexWrap:"wrap",gap:"0.5rem"}}>
                {c.activities.map((act,j)=>(
                  <span key={j} style={{padding:"0.3rem 0.9rem",border:`1px solid ${c.color}35`,fontSize:"0.7rem",color:"var(--body-text)",transition:"all 0.25s",cursor:"default"}}
      ...
      <style>{`
        @media(min-width:860px){.activity-panel{grid-template-columns:1fr 1fr!important}}
...
function SectionNav(){
  const sections=[
    {label:"Our Facilities",id:"facilities"},
  ...
  return(
    <aside style={{width:"clamp(120px,12vw,180px)",flexShrink:0,position:"sticky",top:110,alignSelf:"flex-start",display:"none"}} className="section-sidebar">
      ...
      <div style={{marginTop:"clamp(1.5rem,2vw,2.25rem)",border:"1px solid #e8e6e3",padding:"clamp(1rem,1.5vw,1.4rem)"}}>
        ...
        {[{label:"About Granada",href:"/granada-school/about"},{label:"Admissions",href:"/granada-school/admissions"},{label:"Wellbeing",href:"/granada-school/wellbeing"}].map((l,i)=>(
